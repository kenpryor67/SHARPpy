...
class plotText(backgroundText):
    ...
    def drawIndices(self, qp):
        ...
        ## middle-right column
        y1 = self.ylast + self.tpad
        texts = ['WNDG = ', 'TEI = ', '3CAPE = ', 'MWPI = ', '', 'SigSvr = ']
        indices = [self.wndg, self.tei, tab.utils.INT2STR(self.prof.mlpcl.b3km), tab.utils.INT2STR(self.prof.mwpi), '', self.sigsevere + ' m3/s3']
        for text, index in zip(texts, indices):
