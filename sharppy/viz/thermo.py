...
class plotText(backgroundText):
    ...
    def drawIndices(self, qp):
        ...
        ## middle-right column
        y1 = self.ylast + self.tpad
        texts = ['WNDG = ', 'TEI = ', '3CAPE = ', 'MBURST1 = ', 'MBURST2 = ', 'SigSvr = ']
        indices = [self.wndg, self.tei, tab.utils.INT2STR(self.prof.mlpcl.b3km), tab.utils.INT2STR(self.prof.mburst1), tab.utils.INT2STR(self.prof.mburst2), self.sigsevere + ' m3/s3']
        for text, index in zip(texts, indices):
